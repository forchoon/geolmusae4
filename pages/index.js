  ...
  return(
    <div style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:"16px",padding:"16px 14px",overflow:"hidden",marginBottom:"16px"}}>
      <div style={{marginBottom:"6px"}}>
        ...
        <div style={{fontSize:"13px",color:lc,fontWeight:"700",marginTop:"2px"}}>
      ...
      {dragIdx!==null&&(
        <div style={{marginTop:"8px",padding:"6px 12px",background:"#60a5fa20",border:"1px solid #60a5fa40",borderRadius:"8px",fontSize:"12px",color:"#60a5fa",fontWeight:"700",textAlign:"center"}}>
...
function CoupangBanner({isDark,T}){
  ...
  return(
    <div style={{marginBottom:"20px",borderRadius:"14px",overflow:"hidden",border:`1px solid ${T.border}`,position:"relative"}}>
      <div style={{position:"absolute",top:"9px",right:"10px",zIndex:2,fontSize:"10px",color:T.textMuted,background:T.bgCard,padding:"2px 7px",borderRadius:"4px",border:`1px solid ${T.border}`,fontWeight:"700"}}>광고 · 쿠팡파트너스</div>
      <div onClick={()=>window.open(p.url,"_blank","noopener,noreferrer")} style={{background:T.bgCard,padding:"14px 16px",display:"flex",alignItems:"center",gap:"12px",cursor:"pointer"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:"13px",fontWeight:"800",color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:"2px"}}>{p.title}</div>
          <div style={{fontSize:"12px",color:T.textSub,fontWeight:"700",marginBottom:"5px"}}>{p.desc}</div>
          <div style={{display:"flex",alignItems:"center",gap:"7px"}}>
            <span style={{fontSize:"14px",fontWeight:"800",color:isDark?"#60a5fa":"#2563eb"}}>{p.price}</span>
            <span style={{fontSize:"11px",padding:"2px 7px",background:"rgba(239,68,68,0.12)",color:"#ef4444",borderRadius:"4px",fontWeight:"800"}}>로켓배송</span>
          </div>
        </div>
        <div style={{fontSize:"12px",color:isDark?"#60a5fa":"#2563eb",fontWeight:"800",flexShrink:0}}>구매 →</div>
      </div>
      ...
      <div style={{padding:"5px 12px",background:T.bgDeep,fontSize:"10px",color:T.textMuted,textAlign:"center",fontWeight:"700"}}>이 포스팅은 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.</div>
    </div>
  );
}
...
function AdSenseBanner({isDark,T,isProfit}){
  return(
    <div style={{marginTop:"14px",borderRadius:"12px",overflow:"hidden",border:`1px solid ${T.border}`,position:"relative"}}>
      <div style={{position:"absolute",top:"9px",left:"10px",zIndex:1,fontSize:"10px",color:T.textMuted,background:T.bgCard,padding:"2px 7px",borderRadius:"4px",border:`1px solid ${T.border}`,fontWeight:"700"}}>광고 · Google AdSense</div>
      <div onClick={()=>window.open("https://securities.example.com/open-account","_blank","noopener,noreferrer")} style={{background:T.bgCard,padding:"22px 16px 16px",textAlign:"center",cursor:"pointer"}}>
        ...
        <div style={{fontSize:"15px",fontWeight:"800",color:isDark?"#93c5fd":"#1d4ed8",marginBottom:"4px"}}>{isProfit?"수익 실현, 지금 계좌 개설":"다음 기회는 놓치지 마세요"}</div>
        <div style={{fontSize:"12px",color:T.textSub,marginBottom:"14px",lineHeight:"1.6",fontWeight:"700"}}>{isProfit?"껄무새가 찾아준 종목, 지금 바로 투자해보세요":"다음 엔비디아를 먼저 잡는 계좌"}</div>
        <div style={{display:"inline-flex",alignItems:"center",gap:"6px",background:"linear-gradient(135deg,#2563eb,#1d4ed8)",color:"#fff",fontSize:"14px",fontWeight:"800",padding:"10px 26px",borderRadius:"22px"}}>무료 계좌 개설 →</div>
        <div style={{fontSize:"10px",color:T.textMuted,marginTop:"9px",fontWeight:"700"}}>신규 계좌 개설 시 수수료 혜택 · 광고성 정보</div>
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.88)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",backdropFilter:"blur(16px)",overflowY:"auto"}}>
      <div id="share-card-inner" style={{width:"320px",background:isProfit?"linear-gradient(145deg,#071a0a,#0d2e12)":"linear-gradient(145deg,#1a0505,#2d0808)",borderRadius:"24px",border:`1px solid ${mc}30`,boxShadow:`0 0 60px ${mc}25`,padding:"32px 24px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontSize:"12px",color:mc,opacity:0.7,marginBottom:"3px",fontWeight:"700"}}>{stockName}</div>
        <div style={{fontSize:"11px",color:mc,opacity:0.5,marginBottom:"14px",fontWeight:"700"}}>{investYear}년에 {isProfit?"샀더라면":"팔았더라면"}…</div>
        <div style={{fontSize:"56px",fontWeight:"800",color:mc,letterSpacing:"-2px",lineHeight:1,marginBottom:"6px"}}>{isProfit?"+":""}{returnPct}%</div>
        <div style={{fontSize:"18px",color:"#e8f0ea",fontWeight:"700",marginBottom:"4px"}}>{formatKRW(Math.round(currentValueKRW))}</div>
        <div style={{fontSize:"10px",color:mc,opacity:0.5,marginBottom:"16px",fontWeight:"700"}}>{formatKRW(parseInt(investAmount)*10000)} 투자 → {CURRENT_YEAR}년</div>
        <div style={{fontSize:"11px",color:mc,opacity:0.7,lineHeight:"1.6",fontStyle:"italic",fontWeight:"700"}}>"{msg}"</div>
      ...
      <div style={{display:"flex",gap:"10px",marginTop:"14px",width:"320px"}}>
        <button onClick={()=>{const txt=isProfit?`${stockName} ${investYear}년에 샀더라면 +${returnPct}%! ${char} "${msg}" | stockparrot.kr`:`${stockName} ${investYear}년에 팔았더라면 ${returnPct}%... ${char} "${msg}" | stockparrot.kr`;navigator.clipboard?.writeText(txt).then(()=>alert("클립보드에 복사됐어요! 인스타에 붙여넣기 하세요 🦜"));}} style={{flex:1,padding:"13px",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",borderRadius:"12px",cursor:"pointer",color:"#fff",fontSize:"14px",fontWeight:"800"}}>🔗 텍스트 복사</button>
        <button onClick={handleSaveImage} disabled={saving} style={{flex:1,padding:"13px",background:saving?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.18)",borderRadius:"12px",cursor:saving?"not-allowed":"pointer",color:"#ccc",fontSize:"14px",fontWeight:"800"}}>
          {saving?"⏳ 저장 중…":"📥 이미지 저장"}
        </button>
      </div>
      <button onClick={onClose} style={{marginTop:"10px",background:"none",border:"none",color:"#666",fontSize:"13px",cursor:"pointer",padding:"6px",fontWeight:"700"}}>닫기</button>
    </div>
  );
}
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    calcCompare();
  },[compareStock]);const yt=getYahooTicker(s.ticker);setSelectedStock({ticker:s.ticker,yahooTicker:yt,name:s.name});setCurrency(getCurrency(yt));setResult(null);setInvestYear(Math.floor((getIpoYear(yt)+lastYear)/2));};
  ...
  const snStyle={width:"28px",height:"28px",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",fontWeight:"800",color:"#fff",flexShrink:0,lineHeight:"1",padding:"0",minWidth:"28px",minHeight:"28px"};
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:T.bg,color:T.text,overflowX:"hidden",transition:"background 0.3s,color 0.3s",fontFamily:"'Nanum Gothic','Apple SD Gothic Neo',sans-serif"}}>
        ...
        <div style={{padding:"36px 20px 16px",textAlign:"center",position:"relative"}}>
          <button onClick={()=>setIsDark(d=>!d)} style={{position:"absolute",top:"18px",right:"18px",background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:"20px",padding:"7px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:"6px",color:T.textSub,fontSize:"13px",fontWeight:"700",zIndex:10}}>
            <span>{isDark?"☀️":"🌙"}</span><span>{isDark?"라이트":"다크"}</span>
          </button>
          <div style={{fontSize:"36px",marginBottom:"4px"}}>🦜</div>
          <h1 style={{fontSize:"32px",fontWeight:"800",margin:0,letterSpacing:"-1px",background:isDark?"linear-gradient(135deg,#ffffff 0%,#86efac 60%,#4ade80 100%)":"linear-gradient(135deg,#0d1a10 0%,#16a34a 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>껄무새</h1>
          <p style={{fontSize:"13px",color:T.accent,margin:"6px 0 2px",fontWeight:"700"}}>그때 살걸!!! 그때 팔걸!!! 껄껄껄 🦜</p>
        </div>
        ...
        <div style={{maxWidth:"600px",margin:"0 auto 8px",padding:"0 16px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"10px",padding:"10px 18px",background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:"12px"}}>
            ...
            <span style={{fontSize:"13px",color:T.textSub,fontWeight:"700"}}>USD / KRW</span>
            <div style={{width:"1px",height:"14px",background:T.border}}/>
            {rateLoading?<span style={{fontSize:"13px",color:T.textMuted,fontWeight:"700"}}>조회 중…</span>:<>
              <span style={{fontSize:"17px",fontWeight:"800",color:T.text}}>{usdToKrw.toLocaleString()}<span style={{fontSize:"13px",color:T.textSub,marginLeft:"2px",fontWeight:"700"}}>원</span></span>
              <span style={{fontSize:"11px",fontWeight:"800",padding:"3px 9px",background:`${T.accent}25`,color:T.accent,borderRadius:"6px"}}>실시간</span>
            </>}
          </div>
        </div>
        ...
        <div style={{maxWidth:"600px",margin:"0 auto",padding:"0 16px"}}>
          ...
          <div style={sec}>
            <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"18px"}}>
              ...
              <span style={{fontSize:"17px",fontWeight:"800",color:T.text,letterSpacing:"-0.3px"}}>종목 선택</span>
              <div style={{flex:1,height:"1px",background:T.border}}/>
            </div>
            <div style={{display:"flex",gap:"4px",marginBottom:"16px",background:T.tabBg,borderRadius:"14px",padding:"4px"}}>
              {TABS.map(tab=><button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{flex:1,padding:"11px 4px",background:activeTab===tab.id?T.tabActive:"transparent",border:`1px solid ${activeTab===tab.id?T.borderActive+"60":"transparent"}`,borderRadius:"10px",cursor:"pointer",color:activeTab===tab.id?T.accent:T.tabInactive,fontSize:"13px",fontWeight:"700",transition:"all 0.2s"}}>{tab.label}</button>)}
            </div>
            {/* 검색 */}
            <div style={{marginBottom:"14px",position:"relative"}}>
              <div style={{position:"relative"}}>
                <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={activeTab==="kr"?"삼성전자, 카카오…":activeTab==="index"?"SPY, QQQ…":"코카콜라, KO…"} style={{width:"100%",background:T.inputBg,border:`1px solid ${T.border}`,borderRadius:"12px",padding:"12px 46px 12px 16px",color:T.text,fontSize:"14px",fontWeight:"700",outline:"none"}}/>
              ...
              {showDropdown&&<>
                ...
                <div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:99,background:T.dropdownBg,border:`1px solid ${T.border}`,borderRadius:"12px",marginTop:"4px",overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,0.2)"}}>
                  {searchResults.length===0?<div style={{padding:"14px 16px",color:T.textMuted,fontSize:"13px",fontWeight:"700"}}>검색 결과가 없어요 😢</div>
                  :searchResults.map((s,i)=><div key={s.ticker} onClick={()=>handleSelectSearch(s)} style={{padding:"13px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:i<searchResults.length-1?`1px solid ${T.border}`:"none"}} onMouseEnter={e=>e.currentTarget.style.background=T.bgDeep} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div><span style={{color:T.accent,fontWeight:"800",fontSize:"14px",marginRight:"8px"}}>{s.nameKo||s.name}</span><span style={{color:T.textMuted,fontSize:"12px",fontWeight:"700"}}>{s.name}</span></div><span style={{color:T.textMuted,fontSize:"12px",fontWeight:"700"}}>선택 →</span></div>)}
                </div>
              </>}
            </div>
            <div style={{fontSize:"11px",color:T.accent,letterSpacing:"2px",marginBottom:"10px",fontWeight:"700"}}>{activeTab==="index"?"대표 지수 · ETF":"인기 종목"}</div>
            <div style={{display:"flex",gap:"7px",flexWrap:"wrap",marginBottom:"16px"}}>
              {currentPresets.map(s=><button key={s.ticker} onClick={()=>handleSelectPreset(s)} style={{padding:"8px 13px",background:selectedStock.ticker===s.ticker?T.presetActive:T.presetInactive,border:`1px solid ${selectedStock.ticker===s.ticker?T.borderActive:T.border}`,borderRadius:"8px",cursor:"pointer",color:selectedStock.ticker===s.ticker?T.accent:T.presetInactiveText,fontSize:"13px",fontWeight:"700",transition:"all 0.15s"}}>{s.emoji?`${s.emoji} `:""}{s.name}</button>)}
            </div>
            <div style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:"12px",padding:"14px",marginBottom:"16px"}}>
              <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                <div style={{background:T.presetActive,borderRadius:"8px",padding:"8px 11px",flexShrink:0}}><div style={{color:T.accent,fontWeight:"800",fontSize:"11px"}}>{isUSD?"USD":"KRW"}</div></div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:T.text,fontWeight:"800",fontSize:"15px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{selectedStock.name}</div>
                  <div style={{color:T.textSub,fontSize:"13px",marginTop:"2px",fontWeight:"700"}}>{firstYear}년 상장 · 현재 {priceLoading?"조회 중…":currentPrice?displayPrice(currentPrice):"-"}</div>
                </div>
                {liveReturnPct&&<span style={{color:parseFloat(liveReturnPct)>=0?T.accent:"#f87171",fontSize:"15px",fontWeight:"800",flexShrink:0}}>{parseFloat(liveReturnPct)>=0?"+":""}{liveReturnPct}%</span>}
              </div>
              {isUSD&&<div style={{marginTop:"10px",paddingTop:"10px",borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:"6px"}}>
                <span style={{fontSize:"12px",color:T.textSub,marginRight:"2px",fontWeight:"700"}}>표시 통화</span>
          ...
          <div style={sec}>
            <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"18px"}}>
              ...
              <span style={{fontSize:"17px",fontWeight:"800",color:T.text,letterSpacing:"-0.3px"}}>매수 시점 선택</span>
              <div style={{flex:1,height:"1px",background:T.border}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"14px"}}>
              <div>
                <span style={{fontSize:"32px",fontWeight:"800",color:T.text,letterSpacing:"-1px"}}>{investYear}년</span>
                <span style={{fontSize:"14px",color:T.accent,fontWeight:"700",marginLeft:"8px"}}>오늘({getTodayMMDD()})</span>
                {buyPrice&&!priceLoading&&<span style={{fontSize:"14px",color:T.textSub,fontWeight:"700",marginLeft:"6px"}}>{displayPrice(buyPrice)}</span>}
                <span style={{fontSize:"13px",color:T.textSub,marginLeft:"4px",fontWeight:"700"}}>매수</span>
              </div>
              <div style={{textAlign:"right"}}>
                {priceLoading&&<div style={{fontSize:"13px",color:T.accent,fontWeight:"700"}}>🦜 조회 중…</div>}
                {priceError&&!priceLoading&&<div style={{fontSize:"12px",color:"#f87171",fontWeight:"700"}}>{priceError}</div>}
                {liveReturnPct&&!priceLoading&&<div style={{fontSize:"15px",fontWeight:"800",color:parseFloat(liveReturnPct)>=0?T.accent:"#f87171"}}>{parseFloat(liveReturnPct)>=0?"+":""}{liveReturnPct}%</div>}
              </div>
            </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:"10px"}}>
              <span style={{fontSize:"12px",color:T.textMuted,fontWeight:"700"}}>{firstYear}년</span>
              <span style={{fontSize:"12px",color:T.textMuted,fontWeight:"700"}}>{Math.round((firstYear+lastYear)/2)}년</span>
              <span style={{fontSize:"12px",color:T.accent,fontWeight:"800"}}>오늘</span>
            </div>
          </div>
          ...
          <div style={sec}>
            <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"18px"}}>
              ...
              <span style={{fontSize:"17px",fontWeight:"800",color:T.text,letterSpacing:"-0.3px"}}>투자 금액 설정</span>
              <div style={{flex:1,height:"1px",background:T.border}}/>
            </div>
            <div style={{display:"flex",gap:"7px",flexWrap:"wrap",marginBottom:"12px"}}>
              {amountOptions.map(a=><button key={a} onClick={()=>setInvestAmount(a)} style={{padding:"8px 12px",background:investAmount===a?T.presetActive:T.presetInactive,border:`1px solid ${investAmount===a?T.borderActive:T.border}`,borderRadius:"8px",cursor:"pointer",color:investAmount===a?T.accent:T.presetInactiveText,fontSize:"13px",fontWeight:"700",transition:"all 0.15s"}}>{parseInt(a)>=10000?`${parseInt(a)/10000}억원`:parseInt(a)>=1000?`${parseInt(a)/1000}천만원`:parseInt(a)>=100?`${parseInt(a)}만원`:`${parseInt(a)*10}만원`}</button>)}
            </div>
            <div style={{position:"relative"}}>
              <input type="number" value={investAmount} onChange={e=>setInvestAmount(e.target.value)} style={{width:"100%",background:T.inputBg,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"13px 60px 13px 16px",color:T.text,fontSize:"16px",fontWeight:"700",outline:"none"}} placeholder="직접 입력"/>
              <span style={{position:"absolute",right:"16px",top:"50%",transform:"translateY(-50%)",color:T.textSub,fontSize:"13px",fontWeight:"700"}}>만원</span>
            </div>
          </div>
          ...
          <div style={sec}>
            <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"18px"}}>
              ...
              <span style={{fontSize:"17px",fontWeight:"800",color:T.text,letterSpacing:"-0.3px"}}>수익률 계산</span>
              <div style={{flex:1,height:"1px",background:T.border}}/>
            </div>
            <button onClick={handleCalculate} disabled={loading||priceLoading||!buyPrice} style={{width:"100%",padding:"18px",background:(loading||priceLoading||!buyPrice)?T.presetActive:`linear-gradient(135deg,${T.accentDim},#15803d)`,border:"none",borderRadius:"14px",cursor:(loading||priceLoading||!buyPrice)?"not-allowed":"pointer",color:"#fff",fontSize:"17px",fontWeight:"800",letterSpacing:"-0.3px",transition:"all 0.2s"}}>
          ...
          {result&&(
            <div id="result-section" key={animKey} style={{marginBottom:"48px",animation:"slideUp 0.5s cubic-bezier(0.16,1,0.3,1)"}}>
              <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"18px"}}>
                ...
                <span style={{fontSize:"17px",fontWeight:"800",color:T.text,letterSpacing:"-0.3px"}}>결과</span>
                <div style={{flex:1,height:"1px",background:T.border}}/>
              </div>
              <div style={{background:T.bgResult,border:`1px solid ${result.isProfit?T.accentDim+"80":"#ef444460"}`,borderRadius:"20px",overflow:"hidden",boxShadow:result.isProfit?`0 8px 40px ${T.accent}15`:"0 8px 40px rgba(239,68,68,0.12)"}}>
                ...
                <div style={{padding:"22px 20px 0"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"18px"}}>
                    <div>
                      <div style={{fontSize:"15px",color:T.text,fontWeight:"800"}}>{selectedStock.name}</div>
                      <div style={{fontSize:"13px",color:T.textSub,marginTop:"3px",fontWeight:"700"}}>{result.buyDateStr} → 오늘 · {result.years}년 보유</div>
                    </div>
                    <div style={{background:result.isProfit?`${T.accent}20`:"rgba(239,68,68,0.15)",border:`1px solid ${result.isProfit?T.accent+"50":"#ef444450"}`,borderRadius:"12px",padding:"10px 16px",textAlign:"center"}}>
                      <div style={{fontSize:"24px",fontWeight:"800",color:result.isProfit?T.accent:"#f87171",lineHeight:1}}>{result.isProfit?"+":""}{result.returnPct}%</div>
                      <div style={{fontSize:"11px",color:T.textSub,marginTop:"3px",fontWeight:"700"}}>연평균 수익률 {result.isProfit?"+":""}{result.cagr}%</div>
                    </div>
                  </div>
                  <div style={{background:isDark?"rgba(0,0,0,0.3)":"rgba(0,0,0,0.04)",borderRadius:"14px",padding:"22px 16px 18px",marginBottom:"16px",textAlign:"center",border:`1px solid ${result.isProfit?T.accent+"20":"#ef444425"}`}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"14px",marginBottom:"16px"}}>
                      <div style={{textAlign:"center"}}><div style={{fontSize:"12px",color:T.textSub,marginBottom:"4px",fontWeight:"700"}}>투자 원금</div><div style={{fontSize:"15px",color:T.textSub,fontWeight:"800"}}>{formatKRW(parseInt(investAmount)*10000)}</div></div>
                      <div style={{fontSize:"20px",color:T.textMuted}}>→</div>
                      <div style={{textAlign:"center"}}><div style={{fontSize:"12px",color:T.textSub,marginBottom:"4px",fontWeight:"700"}}>현재 가치</div><div style={{fontSize:"28px",fontWeight:"800",color:result.isProfit?T.accent:"#f87171",letterSpacing:"-0.5px"}}><CountUp key={animKey} target={result.currentValueKRW} duration={1500}/></div>{isUSD&&<div style={{fontSize:"12px",color:T.textMuted,marginTop:"3px",fontWeight:"700"}}>{formatUSD(result.currentValueKRW/usdToKrw)}</div>}</div>
                    </div>
                    <div style={{display:"inline-flex",alignItems:"center",gap:"8px",padding:"8px 18px",background:result.isProfit?`${T.accent}15`:"rgba(239,68,68,0.12)",borderRadius:"20px",border:`1px solid ${result.isProfit?T.accent+"35":"#ef444435"}`}}>
                      ...
                      <span style={{fontSize:"15px",fontWeight:"800",color:result.isProfit?T.accent:"#f87171"}}>{formatKRW(Math.abs(result.profitKRW))} {result.isProfit?"수익":"손실"}</span>
                    </div>
                    <div style={{marginTop:"16px",fontSize:"16px",fontWeight:"800",color:result.isProfit?T.accent:"#f87171"}}>{parrotMsg}</div>
                  </div>
                  <div style={{display:"flex",gap:"8px",marginBottom:"16px"}}>
                    {[{label:"매수가",value:displayPrice(result.buyPrice)},{label:"현재가",value:displayPrice(result.currentPrice)},{label:"연평균 수익률",value:`${result.isProfit?"+":""}${result.cagr}%`,h:true}].map(item=>(
                      <div key={item.label} style={{flex:1,padding:"12px 8px",textAlign:"center",background:item.h?(result.isProfit?`${T.accent}15`:"rgba(239,68,68,0.1)"):isDark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.03)",borderRadius:"10px",border:`1px solid ${item.h?(result.isProfit?T.accent+"35":"#ef444435"):T.border}`}}>
                        <div style={{fontSize:"11px",color:T.textSub,marginBottom:"5px",fontWeight:"700"}}>{item.label}</div>
                        <div style={{fontSize:"13px",fontWeight:"800",color:item.h?(result.isProfit?T.accent:"#f87171"):T.text,wordBreak:"break-all"}}>{item.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={{padding:"0 20px 22px"}}>
                  <div style={{padding:"16px",background:result.isProfit?`${T.accent}10`:"rgba(239,68,68,0.08)",border:`1px solid ${result.isProfit?T.accent+"25":"#ef444425"}`,borderRadius:"12px",textAlign:"center",marginBottom:"14px"}}>
                    <div style={{fontSize:"14px",color:result.isProfit?T.accent:"#f87171",fontStyle:"italic",lineHeight:"1.7",fontWeight:"700"}}>"{witMsgs[animKey%3]}"</div>
                  </div>

                  {/* 📥 이미지로 저장하기 */}
                  <button onClick={()=>setShowShareCard(true)} style={{width:"100%",padding:"14px",background:result.isProfit?`linear-gradient(135deg,${T.accentDim},#15803d)`:"linear-gradient(135deg,#dc2626,#b91c1c)",border:"none",borderRadius:"12px",cursor:"pointer",color:"#fff",fontSize:"15px",fontWeight:"800",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",marginBottom:"16px"}}>
                    📥 이미지로 저장하기
                  </button>

                  {/* 🔀 다른 종목과 비교 — 칩 바로 노출 */}
                  <div style={{marginBottom:"14px"}}>
                    <div style={{fontSize:"13px",color:T.textMuted,fontWeight:"700",marginBottom:"10px",textAlign:"center"}}>다른 종목과 비교해볼까요?</div>
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap",justifyContent:"center",marginBottom:"12px"}}>
                      {[...US_PRESETS,...KR_PRESETS,...INDEX_PRESETS.slice(0,2)].filter(s=>s.ticker!==selectedStock.ticker).slice(0,12).map(s=>(
                        <button key={s.ticker} onClick={()=>setCompareStock(cs=>cs?.ticker===s.ticker?null:s)} style={{padding:"7px 13px",background:compareStock?.ticker===s.ticker?T.presetActive:T.presetInactive,border:`1px solid ${compareStock?.ticker===s.ticker?T.borderActive:T.border}`,borderRadius:"20px",cursor:"pointer",color:compareStock?.ticker===s.ticker?T.accent:T.presetInactiveText,fontSize:"13px",fontWeight:"700",transition:"all 0.15s"}}>
                          {s.name}
                        </button>
                      ))}
                    </div>
                    ...
                    {compareStock&&compareResult&&(
                      <div style={{background:isDark?"rgba(0,0,0,0.2)":"rgba(0,0,0,0.03)",borderRadius:"14px",padding:"16px",border:`1px solid ${T.border}`}}>
                        <div style={{fontSize:"12px",color:T.textMuted,fontWeight:"700",marginBottom:"12px",textAlign:"center"}}>같은 기간 · 같은 금액 {formatKRW(parseInt(investAmount)*10000)}</div>
                          ...
                          {[{name:selectedStock.name,r:result},{name:compareStock.name,r:compareResult}].map((item,i)=>{
                            ...
                            return(
                              <div key={i} style={{flex:1,padding:"16px 12px",background:better?(result.isProfit?`${T.accent}12`:"rgba(239,68,68,0.08)"):"transparent",border:`2px solid ${better?(result.isProfit?T.accent+"60":"#ef444460"):T.border}`,borderRadius:"14px",textAlign:"center",transition:"all 0.3s"}}>
                                {better&&<div style={{fontSize:"12px",color:result.isProfit?T.accent:"#f87171",fontWeight:"800",marginBottom:"6px"}}>👑 승자</div>}
                                <div style={{fontSize:"13px",fontWeight:"800",color:T.text,marginBottom:"8px"}}>{item.name}</div>
                                <div style={{fontSize:"28px",fontWeight:"800",color:parseFloat(item.r.returnPct)>=0?T.accent:"#f87171",letterSpacing:"-1px",lineHeight:1,marginBottom:"6px"}}>{parseFloat(item.r.returnPct)>=0?"+":""}{item.r.returnPct}%</div>
                                <div style={{fontSize:"13px",color:T.textSub,fontWeight:"700"}}>{formatKRW(Math.round(item.r.currentValueKRW))}</div>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {compareStock&&!compareResult&&(
                      <div style={{textAlign:"center",padding:"14px",color:T.textMuted,fontSize:"13px",fontWeight:"700"}}>⏳ 비교 계산 중…</div>
                    )}
                  </div>

                  <div style={{fontSize:"12px",color:T.textSub,textAlign:"center",lineHeight:"1.7",fontWeight:"700",marginBottom:"12px"}}>⚠️ 과거 수익률은 미래를 보장하지 않습니다.</div>
        ...
        <footer style={{maxWidth:"600px",margin:"0 auto",padding:"28px 16px 48px",borderTop:`1px solid ${T.border}`,textAlign:"center"}}>
          ...
          <div style={{fontSize:"15px",fontWeight:"800",background:isDark?"linear-gradient(135deg,#fff,#86efac)":"linear-gradient(135deg,#0d1a10,#16a34a)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:"2px"}}>껄무새 · stockparrot.kr</div>
          <div style={{fontSize:"12px",color:T.textSub,marginTop:"12px",lineHeight:"1.8",fontWeight:"700"}}>⚠️ 본 서비스는 참고용 엔터테인먼트 콘텐츠로, 투자 조언이 아닙니다.<br/>과거 수익률은 미래 성과를 보장하지 않습니다.</div>
          <div style={{marginTop:"16px",fontSize:"12px",color:T.textMuted,fontWeight:"700"}}>© {CURRENT_YEAR} 껄무새 (stockparrot.kr). All rights reserved.</div>
          <div style={{fontSize:"11px",color:T.textFaint,marginTop:"4px",fontWeight:"700"}}>문의: <a href="mailto:to.choon@gmail.com" style={{color:T.accent,textDecoration:"none",fontWeight:"800"}}>to.choon@gmail.com</a></div>
        </footer>
